...
## 业务规则定义1:课程订单收款记录表
1. 当收款方式!= 会员储值卡,系统:自动更新订单表的"已收金额"和"未收金额"
...
执行规则：UPDATE({课程订单信息表},{课程订单信息表.ObjectId} == {订单信息},{课程订单信息表.已收金额},{收款金额} + {课程订单信息表.已收金额},{课程订单信息表.未收金额},{课程订单信息表.未收金额} - {收款金额})
```
2. 当收款方式!= 会员储值卡,系统:自动更新学员表的“储值余额”
```
触发事件：数据生效时
...
执行规则：UPDATE({学员信息表},{学员信息表.ObjectId} == {学员信息} AND {收款方式} == '会员储值卡',{学员信息表.储值余额},{学员信息表.储值余额} - {收款金额})
```
## 业务规则定义2:储值卡充值记录表
系统:自动更新学员表的"储值金额"
```
...
执行规则：UPDATE({学员信息表},{学员信息表.ObjectId} == {学员信息},{学员信息表.储值余额},{学员信息表.储值余额} + {收款金额})
```
